...
class DiaryEntryRepositoryImpl implements DiaryEntryRepository {
  ...
  Future<int> createDiaryEntry(DiaryEntry entry) async {
    return await _database.transaction(() async {
      // 插入日记条目
      final entryId = await _database.into(_database.diaryEntries).insert(
  ...
  Future<bool> updateDiaryEntry(DiaryEntry entry) async {
    ...
    return await _database.transaction(() async {
      // 更新日记条目
      final updated = await (_database.update(_database.diaryEntries)
      ...
      if (updated > 0) {
        ...
        await _addTagsToEntry(entry.id!, entry.tags.map((tag) => tag.id!).toList());
        return true;
      }
  ...
  Future<bool> deleteDiaryEntry(int id) async {
    return await _database.transaction(() async {
      ...
      // 删除日记条目
      final deleteCount = await (_database.delete(_database.diaryEntries)
  ...
  Stream<List<DiaryEntry>> getDiaryEntriesByTags(List<int> tagIds) {
    ...
    return query.watch().asyncMap((rows) async {
      final entries = rows.map((row) => row.readTable(_database.diaryEntries)).toList();
  ...
  Stream<List<DiaryEntry>> getDiaryEntriesByDateRange(
    ...
    return (_database.select(_database.diaryEntries)
        ..where((entry) => entry.createdAt.isBetweenValues(startDate, endDate))
        ..orderBy([(entry) => OrderingTerm.desc(entry.createdAt)]))
        .watch()
        .asyncMap(_mapToEntitiesWithTags);
  }
  ...
  Future<Map<String, int>> getDiaryStatistics() async {
    final totalCount = await _database.select(_database.diaryEntries).get().then((rows) => rows.length);
    ...
    final monthlyCount = await (_database.select(_database.diaryEntries)
        ..where((entry) => entry.createdAt.isBiggerOrEqualValue(firstDayOfMonth)))
        .get()
        .then((rows) => rows.length);
  ...
  Future<bool> toggleFavorite(int id) async {
    ...
    final updateCount = await (_database.update(_database.diaryEntries)
  ...
  Future<bool> addTagToEntry(int entryId, int tagId) async {
    try {
      await _database.into(_database.diaryTags).insert(
  ...
  Future<bool> removeTagFromEntry(int entryId, int tagId) async {
    final deleteCount = await (_database.delete(_database.diaryTags)
      ..where((dt) => dt.diaryEntryId.equals(entryId) & dt.tagId.equals(tagId))).go();
  ...
  Future<List<Tag>> _getTagsForEntry(int entryId) async {
    ...
    return result.map((row) => _mapTagDataToEntity(row.readTable(_database.tags))).toList();
  }
  ...
  /// 将数据库结果映射为包含标签的实体列表
  Future<List<DiaryEntry>> _mapToEntitiesWithTags(List<DiaryEntryData> dataList) async {
    final entitiesWithTags = <DiaryEntry>[];
