  ...
  group('UseCases', () {
    test('CreateDiaryEntryUseCase: 基本成功与输入校验', () async {
      ...
      final r1 = await usecase.execute(
        CreateDiaryEntryParams(title: '', content: 'x'),
      );
      expect(r1.isFailure, true);
      final r2 = await usecase.execute(
        CreateDiaryEntryParams(title: 't', content: ''),
      );
      expect(r2.isFailure, true);

      // 成功：创建并自动创建标签 + 增加使用次数
      final r3 = await usecase.execute(
    ...
    test('UpdateDiaryEntryUseCase: 未找到/成功更新/标签计数变化', () async {
      ...
      final createdId = (await create.execute(
        CreateDiaryEntryParams(title: 'a', content: 'b', tagNames: const ['A']),
      )).dataOrThrow;

      // 未找到
      final nf = await update.execute(
    ...
    test('DeleteDiaryEntryUseCase: 单个与批量', () async {
      ...
      final id1 = (await create.execute(
        CreateDiaryEntryParams(title: 'a', content: 'b', tagNames: const ['A']),
      )).dataOrThrow;
      final id2 = (await create.execute(
        CreateDiaryEntryParams(title: 'c', content: 'd', tagNames: const ['A']),
      )).dataOrThrow;
    ...
    test('GetDiaryEntriesUseCase: 流与统计/按条件查询', () async {
      ...
      final id2 = (await create.execute(
        CreateDiaryEntryParams(title: 'm', content: 'n', tagNames: const ['B']),
      )).dataOrThrow;
      ...
      // tags
      final byTags = await get.getEntriesByTags(const [
        1,
      ]).first; // tag A 可能为 id=1（在内存实现中从1开始累加）
      expect(byTags.isNotEmpty, true);
      // date range（包含今天）
      final byDate = await get
