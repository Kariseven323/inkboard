...
void main() {
  ...
  testWidgets('TagManagementPage 列表渲染/新建/编辑/删除', (tester) async {
    ...
    await tester.enterText(find.descendant(of: createDialog, matching: find.byType(TextField)).at(0), '学习');
    await tester.enterText(find.descendant(of: createDialog, matching: find.byType(TextField)).at(1), '#00BCD4');
    await tester.tap(find.descendant(of: createDialog, matching: find.text('创建')));
    await tester.pumpAndSettle();
    ...
    await tester.enterText(find.descendant(of: editDialog, matching: find.byType(TextField)).at(0), '学习2');
    await tester.tap(find.descendant(of: editDialog, matching: find.text('保存')));
    await tester.pumpAndSettle();
    // 更稳健：校验列表项名称已更新为“学习2”
    ...
    // 删除刚创建的标签（非默认标签可删）
    final deleteIcon = find.widgetWithIcon(IconButton, Icons.delete_outline).last;
