...
void main() {
  ...
  test('create/read/update/delete diary entry with encryption + tags', () async {
    // prepare tag
    final tagId = await harness.tagRepo.createTag(Tag(
      name: '编程',
      color: '#FF5722',
    ...
    final entryId = await harness.diaryRepo.createDiaryEntry(DiaryEntry(
      title: '第一篇',
  ...
  test('search supports title/content (content decrypted path)', () async {
    final now = DateTime.now();
    final id1 = await harness.diaryRepo.createDiaryEntry(DiaryEntry(
      title: '周会纪要',
  ...
  test('date range filter', () async {
    final now = DateTime.now();
    final id = await harness.diaryRepo.createDiaryEntry(DiaryEntry(
      title: '范围内',
      content: 'test',
      createdAt: now,
      updatedAt: now,
    ...
    final list = await harness.diaryRepo
        .getDiaryEntriesByDateRange(now.subtract(const Duration(days: 1)), now.add(const Duration(days: 1)))
        .first;
    expect(list.any((e) => e.id == id), isTrue);

    final empty = await harness.diaryRepo
        .getDiaryEntriesByDateRange(now.add(const Duration(days: 1)), now.add(const Duration(days: 2)))
        .first;
